...
async def CheckAdmin(client: Client, message: Message):
    ...
    if SELF.status not in ranks:
        await message.edit("__من ادمین نیستم!__")
        ...
        if SELF.status is not admin:
            ...
            await message.edit("__هیچ مجوزی برای محدود کردن اعضا وجود ندارد__")
            await asyncio.sleep(2)
            await message.delete()
...
async def CheckReplyAdmin(message: Message):
    ...
    if not message.reply_to_message:
        await message.edit("دستور باید یک پاسخ باشد")
        await asyncio.sleep(2)
        await message.delete()
...
async def RestrictFailed(message: Message):
    await message.edit(f"من نمی توانم {message.command} این کاربر.")
    await asyncio.sleep(2)
    await message.delete()
